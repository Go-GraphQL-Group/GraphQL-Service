...
import (
	...
	model2 "github.com/Go-GraphQL-Group/GraphQL-Service"
...
func GetPeopleByID(ID string) (error, *model2.People) {
...
func GetFilmByID(ID string) (error, *model2.Film) {
	db, err := bolt.Open("./data/data.db", 0600, nil)
	CheckErr(err)
	defer db.Close()
...
func GetPlanetByID(ID string) (error, *model2.Planet) {
	db, err := bolt.Open("./data/data.db", 0600, nil)
	CheckErr(err)
	defer db.Close()
...
func GetSpeciesByID(ID string) (error, *model2.Specie) {
	db, err := bolt.Open("./data/data.db", 0600, nil)
	CheckErr(err)
	defer db.Close()
...
func GetStarshipByID(ID string) (error, *model2.Starship) {
	db, err := bolt.Open("./data/data.db", 0600, nil)
	CheckErr(err)
	defer db.Close()
...
func GetVehicleByID(ID string) (error, *model2.Vehicle) {
	db, err := bolt.Open("./data/data.db", 0600, nil)
	CheckErr(err)
	defer db.Close()
